    ...
    const loadVault = async () => {
      if (!vaultId || !entryCid) {
        ...
        if (!loading && client) {
          ...
          if (!vault) {
            showError(
              'The vault associated with the shared file was not found'
            );
          } else {
            ...
            if (!manifest) {
              showError(
                'The manifest associated with the shared file was not found'
              );
            } else {
              const matchedEntry = manifest?.entries.filter(
                (e) => e.cid === entryCid
              );
              if (matchedEntry) {
